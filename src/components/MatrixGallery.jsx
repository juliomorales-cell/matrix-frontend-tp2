...
const matrixImages = [
  ...
  { src: '/cual-es-el-nombre-de-la-ciudad-subterranea.jpg', alt: 'Ciudad Subterránea' },
  { src: '/Matrix_Revolution.jpg', alt: 'Matrix Revolution' },
...
export default function MatrixGallery() {
  ...
  useEffect(() => {
    if (audioRef.current && audioPlaying) {
      audioRef.current.play().catch(err => {
        console.log('El navegador pide interacción previa para iniciar el sonido:', err);
        setAudioPlaying(false); // Si lo bloquea, actualizamos el botón a "mutado"
      });
    }
  ...
  const nextImage = (e) => {
    ...
    setSelectedIndex((prev) => (prev === matrixImages.length - 1 ? 0 : prev + 1));
    setIsZoomed(false);
  };

  const prevImage = (e) => {
    if (e) e.stopPropagation();
    setSelectedIndex((prev) => (prev === 0 ? matrixImages.length - 1 : prev - 1));
    setIsZoomed(false);
  };
  ...
  const toggleAudio = () => {
    ...
    if (audioPlaying) {
      ...
      audioRef.current.play().catch(err => console.log('Reproducción bloqueada:', err));
      ...
      {selectedIndex !== null && (
        ...
        >
          ...
          <div className="relative w-full max-w-4xl h-[75vh] flex flex-col items-center justify-center z-40">
            <div className={`flex-1 w-full flex items-center justify-center overflow-auto rounded ${isZoomed ? 'items-start' : ''}`}>
              <img
                src={matrixImages[selectedIndex].src}
                alt={matrixImages[selectedIndex].alt}
                className={`max-w-full object-contain rounded border border-green-500 shadow-2xl shadow-green-500/30 transition-transform duration-300 ${isZoomed ? 'scale-[1.7] cursor-zoom-out origin-center' : 'scale-100 cursor-zoom-in max-h-[65vh]'}`}
                onClick={(e) => {
                  e.stopPropagation();
            ...
            <p className="mt-4 text-green-400 text-sm tracking-widest bg-black/80 px-4 py-2 border border-green-900 rounded uppercase shrink-0">
              {matrixImages[selectedIndex].alt} — [{selectedIndex + 1} / {matrixImages.length}]
            </p>
          </div>
        </div>
      )}
