...
export default function Bitacora() {
  const [entradas, setEntradas] = useState([
    {
      id: 1,
      texto: "> ASIGNACIÓN DE ROLES Y METODOLOGÍA: Se ha establecido el uso de Trello para la gestión del proyecto mediante tableros Kanban. El flujo de trabajo coordinado se basa en GitFlow, utilizando ramas 'feature' para nuevas funcionalidades, 'develop' para integración constante y 'main' para despliegues estables.",
      fecha: new Date(Date.now() - 86400000 * 2).toLocaleString() // Hace 2 días
    },
    {
      id: 2,
      texto: "> JUSTIFICACIÓN DE MIGRACIÓN A REACT: La estructura inicial basada en HTML y JS Vanilla ha sido reemplazada. La migración a React permite aprovechar una arquitectura de componentes reutilizables, un manejo de estados dinámicos eficiente y el Virtual DOM, lo que optimiza drásticamente el rendimiento del sistema de Zion.",
      fecha: new Date(Date.now() - 86400000).toLocaleString() // Hace 1 día
    },
    {
      id: 3,
      texto: "> INICIALIZACIÓN DEL SISTEMA: Enlaces neuronales estables. Conexión con la base de datos de la tripulación establecida con éxito. Sistema listo.",
  ...
  const iniciarSecuencia = () => {
    ...
    if (audioNeoRef.current) {
      ...
      audioNeoRef.current.play().catch(() => console.log("Audio neo-hablando bloqueado"));
    }
  ...
  const handleNeoEnded = () => {
    ...
    if (audioWakeRef.current) {
      ...
      audioWakeRef.current.play().catch(() => console.log("Audio wake-up bloqueado"));
  ...
  const startWakeUpFadeIn = () => {
    ...
    fadeIntervalRef.current = setInterval(() => {
      if (audioWakeRef.current) {
        const newVolume = Math.min(audioWakeRef.current.volume + step, targetVolume);
  ...
  return (
    <div 
      ...
      onClick={() => { if (step === 0) iniciarSecuencia(); }}
    >
      <MatrixRain isPaused={step === 1} isInBackground={step === 2} />

      {/* 🎵 REPRODUCTORES DE AUDIO */}
      <audio ref={audioNeoRef} src="/neo-hablando.mp4" onEnded={handleNeoEnded} />
      ...
      {step === 1 && (
        ...
        >
          <div style={{ textAlign: 'center', animation: 'glitchPulse 0.8s infinite ease-in-out' }}>
            <h1
              style={{
      ...
      {step === 2 && (
        ...
        >
          ...
          <div className="border border-green-500 bg-black/80 p-6 rounded shadow-[0_0_15px_rgba(0,255,65,0.2)] flex justify-between items-center">
            <div>
              <h2 className="text-2xl md:text-3xl font-bold text-green-500 mb-1 font-mono">&gt; BITÁCORA DE EVENTOS</h2>
              <p className="text-green-700 text-sm font-mono">Registro manual de anomalías y sucesos en el sistema.</p>
            </div>
            <button
              onClick={toggleMute}
          ...
          <form onSubmit={agregarEntrada} className="flex flex-col md:flex-row gap-3">
            <input 
              type="text" 
              value={nuevaEntrada}
            ...
            {entradas.map(entry => (
              ...
              >
                <div className="flex-1 font-mono">
                  <p className="text-white text-sm leading-relaxed">{entry.texto}</p>
